...
#ifndef T1
!T1:syntax,firstprivate,runtime,compute,V:3.4-
...
#ifndef T2
!T2:syntax,firstprivate,runtime,compute,V:3.4-
...
#ifndef T4
!T4:syntax,firstprivate,runtime,compute,V:3.4-
...
! named constant in CREATE clause (device allocation only); used on device (read-only)
...
#ifndef T7
!T7:syntax,data-clause,runtime,compute,V:3.4-
! named constant in PRESENT_OR_COPYIN data clause; used on device (read-only)
      LOGICAL FUNCTION test7()
        IMPLICIT NONE
        INCLUDE "acc_testsuite.Fh"
        INTEGER :: i, errors
        REAL(8), DIMENSION(LOOPCOUNT) :: a, c
        INTEGER, PARAMETER :: OFF = 9

        errors = 0
        CALL RANDOM_NUMBER(a)
        c = 0.0D0

        !$acc data copyin(a(1:LOOPCOUNT)) present_or_copyin(OFF) copy(c(1:LOOPCOUNT))
          !$acc parallel loop present(a(1:LOOPCOUNT), c(1:LOOPCOUNT))
          DO i = 1, LOOPCOUNT
            c(i) = a(i) + DBLE(OFF)
          END DO
