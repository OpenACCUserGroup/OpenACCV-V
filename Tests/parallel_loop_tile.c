...
int test(){
    ...
    {
        #pragma acc parallel loop tile((int) tile_arg/10, tile_arg, tile_arg*2)
        for (int x = 0; x < n; ++x){
            for (int y = 0; y < n; ++y){
                for (int z = 0; z < n; ++z){
...
int main()
{
  ...
  printf("##############################################\n");
  printf("Testing parallel_loop_tile\n\n");

  fprintf(logFile,"######## OpenACC Validation Suite V %s #####\n", ACCTS_VERSION );
  fprintf(logFile,"## Repetitions: %3d                       ####\n",REPETITIONS);
  fprintf(logFile,"## Array Size : %.2f MB                 ####\n",ARRAYSIZE * ARRAYSIZE/1e6);
  fprintf(logFile,"##############################################\n");
  fprintf(logFile,"Testing parallel_loop_tile\n\n");

  for ( i = 0; i < REPETITIONS; i++ ) {
    fprintf (logFile, "\n\n%d. run of parallel_loop_tile out of %d\n\n",i+1,REPETITIONS);
    if (test() == 0) {
