...
// - T3: uses side-effecting expressions in directive arguments; we do NOT rely on the side effects;
...
int test3(){
    ...
    for (int k = 0; k < n; ++k){
        a[k] = rand() / (real_t)(RAND_MAX / 10);
        b[k] = rand() / (real_t)(RAND_MAX / 10);
        c[k] = 0;
    }
